 ...
 * alumno de ESE grupo, con:
 *   · Selector de GRUPO (por si quieres comparar otro sin salir).
 *   · Selector de ESCENARIO de alumno para demostraciones en clase:
 *       🌱 Recién empieza · 🐢 Practica poco · 🔥 El que más practica
 *
 * No hay botón flotante: el botón vive dentro del grupo y llama a
 *   window.JUCUM_STUDENT_PREVIEW.open(groupId)
 *
 * Seguridad: no crea cuentas, no toca el login, no escribe en Supabase
 * (se anulan las escrituras y se restauran al salir); el alumno es ficticio y
 * se elimina sin rastro al pulsar "✕ Salir".
...
(function () {
  ...
  /* ── escenarios: alimentan stats + progreso del dashboard real ─────── */
  ...
  var SCEN = [{ k: 'nuevo', t: '🌱 Recién empieza' }, { k: 'poco', t: '🐢 Practica poco' }, { k: 'top', t: '🔥 El que más practica' }];
  ...
  function JucumPreviewRoot() {
    ...
    enterRef.current = function (g) {
      ...
      gate(true); apply(target, 'nuevo');
      try { document.body.style.overflow = 'hidden'; } catch (e) {}
      setGid(target); setScen('nuevo'); setActive(true);
    };
