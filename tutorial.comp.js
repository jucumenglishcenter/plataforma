 ...
 * ───────────────────────────────────────────────────────────────────────────
 * Recorrido paso a paso que RESALTA las áreas reales (spotlight) con
 * Siguiente/Anterior. Se ofrece la primera vez que el alumno entra (puede
 * aceptar o rechazar) y puede reabrirse desde el menú (🎓 Tour).
 *
 * ACTUALIZADOR (2 vías):
 *  • TUT_VERSION: versión del contenido del recorrido. Cuando la plataforma
 *    cambie y actualicemos los pasos, subimos este número → el tutorial se
 *    vuelve a ofrecer a todos automáticamente.
 *  • Campaña (nube): el profesor toca "🔁 Volver a mostrarlo a todos" en su
 *    panel → sube 'tutorial_campaign' en app_settings → se re-ofrece a todos.
 *
 * Exporta a window: StudentTutorial, TutorialAdminCard, JUCUM_TUTORIAL.
 */
(function () {
  const { useState, useEffect, useLayoutEffect, useCallback } = React;

  const TUT_VERSION = 2; // ⬆ subir cuando cambie la plataforma/los pasos
  ...
  /* ── Campaña en la nube (app_settings.tutorial_campaign) ── */
  ...
  /* ── Spotlight (agujero + tarjeta, montado en <body>) ── */
  function Spotlight({ steps, idx, rects, onNext, onPrev, onClose }) {
    const st = steps[idx];
    const rect = rects[st.target];
    if (!rect) return null;
    const below = rect.top < window.innerHeight / 2;
    const tipStyle = below
      ? { top: rect.top + rect.height + 18, left: Math.min(Math.max(rect.left, 12), Math.max(12, window.innerWidth - 312)) }
      : { top: Math.max(rect.top - 210, 12), left: Math.min(Math.max(rect.left, 12), Math.max(12, window.innerWidth - 312)) };
    const last = idx === steps.length - 1;
    return ReactDOM.createPortal(
      <>
        <div style={{ position: 'fixed', top: rect.top - 6, left: rect.left - 6, width: rect.width + 12, height: rect.height + 12, borderRadius: 14, boxShadow: '0 0 0 9999px rgba(15,23,42,.74)', border: '3px solid #FFD54A', zIndex: 100000, pointerEvents: 'none' }}></div>
  ...
  /* ── Tutorial del alumno (montar en el panel de inicio del alumno) ── */
  function StudentTutorial({ student }) {
    ...
    // ¿Debe ofrecerse? (primera vez, versión nueva o campaña reprogramada)
    useEffect(() => {
      let alive = true;
      (async () => {
    ...
    const start = () => { setIdx(0); window.scrollTo(0, 0); setPhase('running'); setTimeout(measure, 250); };
    const close = () => { markSeen(); setPhase('idle'); };

    // Reapertura desde fuera (botón 🎓 Tour del menú)
    useEffect(() => { window.JUCUM_TUTORIAL = window.JUCUM_TUTORIAL || {}; window.JUCUM_TUTORIAL.start = start; return () => { if (window.JUCUM_TUTORIAL) delete window.JUCUM_TUTORIAL.start; }; }, []);

    // saltar pasos cuyo elemento no existe en pantalla
    const visibleSteps = phase === 'running' ? STEPS.filter(s => rects[s.target]) : STEPS;
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', zIndex: 100000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: "'Nunito',sans-serif" }} onClick={close}>
          <div style={{ background: '#fff', borderRadius: 18, width: '100%', maxWidth: 400, padding: '26px 24px', textAlign: 'center', boxShadow: '0 24px 60px rgba(0,0,0,.35)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              ...
              <button onClick={start} style={{ border: 'none', background: 'linear-gradient(135deg,#FF7FB2,#FF5FA0)', color: '#fff', fontFamily: 'inherit', fontWeight: 800, fontSize: 13, borderRadius: 20, padding: '10px 20px', cursor: 'pointer' }}>¡Sí, muéstrame! →</button>
            </div>
          </div>
        </div>, document.body);
    }

    if (phase === 'running' && visibleSteps.length > 0) {
      const safeIdx = Math.min(idx, visibleSteps.length - 1);
      return <Spotlight steps={visibleSteps} idx={safeIdx} rects={rects}
        onNext={() => setIdx(i => Math.min(i + 1, visibleSteps.length - 1))}
  ...
  /* ── Tarjeta del profesor: reprogramar el tutorial para todos ── */
  function TutorialAdminCard() {
    ...
    const doIt = async () => {
      ...
      setBusy(true); setMsg('');
      try { const n = await bumpCampaign(); setMsg('✅ Listo. El tutorial se ofrecerá a todos de nuevo (campaña #' + n + ').'); }
    ...
    return (
      <div className="scard" style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-soft)', fontWeight: 600, lineHeight: 1.45 }}>Se ofrece solo la primera vez (y cuando la plataforma se actualiza). Pruébalo con "👁 Ver como alumno". {msg && <b style={{ color: '#1F3A8A' }}>{msg}</b>}</div>
        </div>
